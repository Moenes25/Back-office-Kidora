...
import { lineOptions, areaOptions, radarOptions } from "components/KpiBar";
import AlertsPanel from "components/AlertsPanel";
import AppointmentPlanner from "components/calendar/AppointmentPlanner";

// icons
import { PiUsersThree, PiBaby, PiChalkboardTeacher } from "react-icons/pi";
import { MdChildCare, MdPersonAddAlt1 } from "react-icons/md";
...
function EntityFilter({
  value,
  onChange,
  counts = { creches: 0, garderies: 0, ecoles: 0 },
}) {
  ...
  const options = [
    {
      v: "creches",
      label: "Crèches",
      count: counts.creches ?? 0,
      color: "bg-indigo-500",
    },
    {
      v: "garderies",
      label: "Garderies",
      count: counts.garderies ?? 0,
      color: "bg-amber-500",
    },
    {
      v: "ecoles",
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      select(options[Math.max(0, hover)].v);
    }
  };
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        ...
        aria-expanded={open}
        className="
          border-black/10 group inline-flex items-center gap-2 rounded-2xl border
          bg-white/80 px-3 py-1.5 text-sm font-semibold shadow-sm
          transition hover:bg-white focus:outline-none focus-visible:ring-2
          focus-visible:ring-sky-300
        "
        style={{
          // petit relief 3D et halo discret
          boxShadow:
            "inset 0 -1px 0 rgba(2,6,23,.06), 0 6px 18px -10px rgba(2,6,23,.18)",
        }}
      >
        ...
        {/* compteur */}
        <span className="bg-black/5 ml-1 rounded-full px-2 text-[11px] font-bold">
          {current.count}
        </span>
      ...
      {open && (
        <div
          ...
          aria-activedescendant={
            hover >= 0 ? `opt-${options[hover].v}` : undefined
          }
          className="
            border-black/10 anim-pop absolute right-0 z-20 mt-2 w-56 rounded-2xl border
            bg-white/95 shadow-[0_18px_45px_-22px_rgba(2,6,23,.35)]
            backdrop-blur
          "
        >
          {/* petite pointe */}
          <span className="border-black/10 absolute -top-2 right-6 h-3 w-3 rotate-45 border-l border-t bg-white" />
                ...
                <li key={o.v} id={`opt-${o.v}`}>
                  <button
                    ...
                    className={[
                      ...
                      active ? "bg-sky-50/70 ring-1 ring-sky-200" : "",
                    ].join(" ")}
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      {active && (
                        ...
                        >
                          <path
                            fill="currentColor"
                            d="M7.5 13.3l-3-3 1.1-1.1 1.9 1.9 5.4-5.4 1.1 1.1z"
                          />
                        </svg>
                      )}
                    </span>
                    <span className="bg-black/5 rounded-full px-2 text-[11px] font-bold">
                      {o.count}
                    </span>
                  </button>
                </li>
    ...
    <div className="animate-fadeIn">
      ...
      <div className="mt-3 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 3xl:grid-cols-6">
        <WidgetKids
          ...
          subtitle={kpis.totalClientsActifs}
        />
        <WidgetKids
          stacked
          tone="sunny"
        ...
        <WidgetKids
          ...
          delay={300}
          icon={<PiBaby />}
          title="Crèches actives"
          subtitle={kpis.creches}
        />
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 md:grid-cols-3">
        ...
        <Card extra="relative p-5 rounded-2xl shadow-sm hover:-translate-y-0.5 transition">
          ...
          <ApexChart
            ...
            series={[
              {
                name: "Garderies",
                data: [12, 15, 14, 18, 20, 22, 24, 23, 19, 17, 16, 18],
              },
              {
                name: "Crèches",
                data: [10, 12, 11, 14, 16, 18, 19, 18, 16, 15, 14, 15],
              },
        ...
        <Card extra="relative p-5 rounded-2xl shadow-sm hover:-translate-y-0.5 transition">
          ...
          <PieChart
            series={[
              [12, 15, 14, 18, 20, 22, 24, 23, 19, 17, 16, 18].reduce(
                (a, b) => a + b,
                0
              ), // Garderies
              [10, 12, 11, 14, 16, 18, 19, 18, 16, 15, 14, 15].reduce(
                (a, b) => a + b,
                0
              ), // Crèches
        ...
        <Card extra="relative p-5  rounded-2xl shadow-sm hover:-translate-y-0.5 transition">
          ...
          <ApexChart
            ...
            series={[
              {
                name: "Garderies",
                data: [68, 70, 72, 74, 76, 78, 80, 79, 77, 75, 73, 72],
              },
              {
                name: "Crèches",
                data: [64, 66, 68, 70, 72, 74, 76, 75, 73, 71, 69, 68],
              },
              {
                name: "Écoles",
                data: [58, 60, 62, 64, 66, 68, 70, 69, 67, 65, 63, 62],
              },
            ]}
            height={360}
          />
        ...
        <Card extra="relative p-5  rounded-2xl shadow-sm hover:-translate-y-0.5 transition">
          ...
          <ApexChart
            ...
            options={{
              ...
              colors: TYPE_COLORS,
              xaxis: {
                categories: ["Qualité", "Support", "Prix", "Fiabilité", "UX"],
              },
            }}
            series={[
...
"
      >
        ...
        <h3 className="text-transparent mb-4 bg-gradient-to-r from-indigo-500 via-purple-500 to-sky-400 bg-clip-text text-lg font-extrabold">
  ...
  `}
        ...
        >
          ...
          <span
            ...
            style={{
              background:
                "radial-gradient(700px 340px at 20% 0%, rgba(99,102,241,.18), transparent 60%)",
            }}
          />
          ...
          <span
            ...
            aria-hidden
          >
            <span className="alarm-scan block h-full w-[40%]" />
          </span>
          ...
          <div className="flex items-start justify-between gap-4">
            <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wide text-gray-500">
              {/* point d’état */}
              <span
                className="bg-rose-500 inline-block h-2 w-2 rounded-full"
                style={{
                  animation:
                    alarm === "critical"
                      ? "alarm-blink .9s steps(2,end) infinite"
                      : alarm === "warn"
                      ? "alarm-blink 1.4s steps(2,end) infinite"
                      : "none",
                }}
              />
              Prochaine expiration —{" "}
              <span className="normal-case text-gray-700">
                {typeLabel[cardFilter]}
              </span>
            </p>
          ...
          <h3
            className="text-transparent via-violet-600 mt-2 inline-flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-sky-500 bg-clip-text text-2xl font-black leading-tight"
            style={{
              animation:
                alarm === "critical"
                  ? "alarm-wiggle .36s ease-in-out 3"
                  : "none",
            }}
          >
            {/* petite sirène si critique */}
            {alarm === "critical" && (
              <span className="bg-rose-100 text-rose-600 ring-rose-200 inline-grid h-6 w-6 place-items-center rounded-md ring-1">
                🔔
              </span>
            )}
            {nextExp.nom}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
            <span className="border-black/10 inline-flex items-center gap-2 rounded-xl border bg-white/70 px-3 py-1 font-semibold text-gray-700">
              <svg
                width="16"
                height="16"
                viewBox="0 0 24 24"
            ...
            <span className="bg-rose-50 text-rose-700 ring-rose-200 inline-flex items-center gap-2 rounded-xl px-3 py-1 font-semibold ring-1">
              <svg width="16" height="16" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M7 2v2H5a2 2 0 0 0-2 2v2h18V6a2 2 0 0 0-2-2h-2V2h-2v2H9V2H7Zm14 8H3v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V10Zm-4 3h-4v4h4v-4Z"
                />
              </svg>
              expire le <span className="ml-1 font-black">{nextExp.date}</span>
            </span>
          </div>
          ...
          <div
            className={[
              "mt-3 flex items-center gap-2 rounded-2xl px-3 py-2 text-sm font-semibold",
              alarm === "critical"
                ? "border-rose-300 bg-rose-50/80 text-rose-700 border"
                : alarm === "warn"
                ? "border border-amber-300 bg-amber-50/80 text-amber-800"
                : "border border-emerald-200 bg-emerald-50/80 text-emerald-700",
            ].join(" ")}
          >
            <span className="bg-current inline-block h-2 w-2 rounded-full opacity-70" />
            {alarm !== "ok" ? "⏰" : "✅"} {nextExp.restant} restant
          </div>

          {/* Barre de progression / activité – couleur selon alarme */}
          <div className="bg-black/10 mt-5 h-1.5 w-full overflow-hidden rounded-full">
            <span
          ...
          <div className="relative z-10 overflow-x-auto rounded-xl">
            {tableRows.length === 0 ? (
              ...
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100 text-xs font-semibold uppercase tracking-wide text-gray-500 ">
                    <th className="px-3 py-3">Nom</th>
                    <th className="px-3 py-3">Ville</th>
                    <th className="px-3 py-3">Enfants</th>
                    <th className="px-3 py-3">Licence</th>
                    <th className="px-3 py-3">Revenus</th>
                  </tr>
                </thead>
                <tbody>
                  {visibleRows.map((c, i) => (
                    <tr
                      key={`${c.nom}-${start + i}`}
                      className="animate-[fadeIn_.35s_ease-out_both] border-b transition-all last:border-0 hover:bg-gray-50"
                      style={{ animationDelay: `${i * 90}ms` }}
                    >
                      <td className="px-3 py-3 font-bold">{c.nom}</td>
                      <td className="px-3 py-3">{c.ville}</td>
                      <td className="px-3 py-3">{c.enfants}</td>
                      ...
                      <td className="px-3 py-3 font-medium">{c.revenue}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex items-center justify-between gap-3 p-3">
              ...
              <nav className="border-black/10 relative inline-flex items-center gap-1 rounded-2xl border bg-white/70 px-1.5 py-1 shadow-sm backdrop-blur-sm">
                {/* curseur animé */}
                <span
                  className="bg-black/[.04] absolute bottom-1 top-1 rounded-xl transition-all duration-300"
                  style={{
